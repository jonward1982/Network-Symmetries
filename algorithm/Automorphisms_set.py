...
def GetAutomorphism(Si, Sj, G, pi, *args):
    ...
    # Create partitions based on state Si
    # For each cell in partition pi, create two new cells:
    # one with vertices having state 0 and one with vertices having state 1
    pit = [[u for u in cell if Si[u] == 0] for cell in pi] + [
        [u for u in cell if Si[u] == 1] for cell in pi
    ]

    # Create partitions based on state Sj
    # Similarly, create new cells based on state Sj
    pib = [[u for u in cell if Sj[u] == 0] for cell in pi] + [
        [u for u in cell if Sj[u] == 1] for cell in pi
    ]
    ...
    if verbose:
        # Print states and partitions for debugging
    ...
    # Create ordered partition pair
    opp = OPPs_set.OrderedPartitionPair(pit, pib, G)
...
class levelinformation(object):
    ...
    def resetlevel(self):
        # Reset the information for the current level
        if self.level > 0:
    ...
    def nextnode_or_decreaselevel(self, opp):
        ...
        while stilllooking:
            if self.nodeind < len(opp.b.pi[self.cellind]) - 1:
                ...
                if self.verbose:
                    print(
                        "Level {}. Next node is {}. OPP:".format(
                            self.level, opp.b.pi[self.cellind][self.nodeind]
                        )
                    )
                    print(opp)
            elif self.level > 0:
    ...
    def create_opp(self, opp):
        ...
        """
        # Reset the partition
        return OPPs_set.OrderedPartitionPair(
...
def mapping(opp, verbose):
    ...
    if opp.IsUnit:
        perm = OPPs_set.GetPermutation(opp)
        if OPPs_set.IsAutomorphism(opp.G, perm):
            if verbose:
                print("\nFound automorphism")
                print(opp)
            return perm
    ...
    while notdone:
        ...
        if verbose:
            print(
                "\nLevel {}. Mapping top vertex {} to bottom vertex {} in cell {}".format(
                    levelinfo.level, u, v, levelinfo.cellind
                )
            )
            print("Split OPP:")
            print(opp)
